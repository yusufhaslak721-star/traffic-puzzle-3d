...
let release=false,timer=null,audioCtx=null,holdTimer=null;

const style=document.createElement('style');
style.textContent=`
#impactHold{position:fixed;inset:0;z-index:9;pointer-events:none;display:none;align-items:center;justify-content:center;background:radial-gradient(circle at center,rgba(255,196,74,.06) 0 16%,rgba(255,82,52,.12) 30%,rgba(0,0,0,.05) 62%,rgba(0,0,0,.18) 100%);box-shadow:inset 0 0 0 0 rgba(255,76,48,0)}
...
function skidAndImpact(){
  ...
  const hit=c.createOscillator(),hg=c.createGain();
  hit.type='square';hit.frequency.setValueAtTime(115,t+.18);hit.frequency.exponentialRampToValueAtTime(48,t+.48);
  hg.gain.setValueAtTime(.0001,t+.18);hg.gain.exponentialRampToValueAtTime(.2,t+.195);hg.gain.exponentialRampToValueAtTime(.0001,t+.52);
  hit.connect(hg);hg.connect(c.destination);hit.start(t+.18);hit.stop(t+.56);
}
function isCrash(){const s=(title?.textContent||'').toLowerCase();return s.includes('çarp')||s.includes('crash')}
function cancelHold(){if(timer){clearTimeout(timer);timer=null}if(holdTimer){clearTimeout(holdTimer);holdTimer=null}release=false;hold.classList.remove('visible')}
function showHold(){
  label.textContent=document.documentElement.lang==='en'?'CRASH!':'ÇARPIŞMA!';
  hold.classList.add('visible');
  skidAndImpact();
  if(holdTimer)clearTimeout(holdTimer);
  holdTimer=setTimeout(()=>{hold.classList.remove('visible');holdTimer=null},1900);
}

for(const b of[homeBtn,retryBtn,nextBtn])b?.addEventListener('click',cancelHold,{capture:true});
...
new MutationObserver(()=>{
  ...
  showHold();
  timer=setTimeout(()=>{timer=null;release=true;result.classList.add('visible')},2000);
