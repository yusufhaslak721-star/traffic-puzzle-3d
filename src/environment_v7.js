...
function routeStartPoint(v){
  ...
  for(let i=0;i<seg.length;i++){
    if(target<=seg[i]||i===seg.length-1){
      ...
      return new THREE.Vector3(THREE.MathUtils.lerp(pts[i][0],pts[i+1][0],t),.6,THREE.MathUtils.lerp(pts[i][1],pts[i+1][1],t))
    }
    target-=seg[i]
  }
...
function fitLevel(level,camera){
  const pts=[];
  for(const n of level?.network?.nodes||[])if(n.intersection)pts.push([n.x,n.z]);
  for(const v of level?.vehicles||[]){const p=routeStartPoint(v);pts.push([p.x,p.z])}
  ...
  const h=23+span*.36,d=18+span*.32,flip=(level.number||1)%4;
  camera.fov=57;
  camera.position.set(cx+(flip===1?d:flip===3?-d:d*.82),h,cz+(flip===2?-d:d));
...
export function createEnvironment(ctx){
  ...
  let activeLevel=null,lastAspect=ctx.camera.aspect;
  return{
    build(level){
      activeLevel=level;
      base.build(level);
      ctx.scene.fog=null;
      fitLevel(level,ctx.camera);
      lastAspect=ctx.camera.aspect
    },
    update(dt,now){
      base.update(dt,now);
      if(activeLevel&&Math.abs(ctx.camera.aspect-lastAspect)>.01){
        fitLevel(activeLevel,ctx.camera);
        lastAspect=ctx.camera.aspect
      }
    }
  }
