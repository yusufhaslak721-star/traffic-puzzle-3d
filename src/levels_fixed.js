...
function startKey(v){const p=v.route.points,a=p[0],b=p[1],dx=b[0]-a[0],dz=b[1]-a[1],l=Math.hypot(dx,dz)||1;return`${a[0].toFixed(2)},${a[1].toFixed(2)}:${(dx/l).toFixed(2)},${(dz/l).toFixed(2)}`}
...
function stabilizeStarts(level){
  ...
  for(const group of groups.values()){
    ...
    // Keep queues readable: front car stays clear of the junction, following cars sit behind it.
    ...
    // Longer vehicles are placed first so trucks/emergency vehicles cannot hide smaller cars.
    ...
    for(let i=0;i<allowed.length;i++){
      const v=allowed[i],total=routeLength(v.route.points);
      const along=Math.max(minFromBoundary,firstSeg-frontClear-i*spacing);
      v.start=+Math.min(.48,Math.max(.012,along/Math.max(total,1))).toFixed(4);
      keep.add(v.id)
    }
  }
  level.vehicles=level.vehicles.filter(v=>keep.has(v.id));
  return level
}
function pointAtStart(v){
  const p=v.route.points,total=routeLength(p);let left=(v.start||0)*total;
  for(let i=1;i<p.length;i++){const d=distance(p[i-1],p[i]);if(left<=d||i===p.length-1){const t=d?left/d:0;return[p[i-1][0]+(p[i][0]-p[i-1][0])*t,p[i-1][1]+(p[i][1]-p[i-1][1])*t]}left-=d}
  return[...p[0]]
}
function removeResidualOverlaps(level){
  const out=[];
  for(const v of level.vehicles){const p=pointAtStart(v);let blocked=false;for(const q of out){const qp=pointAtStart(q),safe=(vehicleLength(v.type)+vehicleLength(q.type))*.5+.55;if(distance(p,qp)<safe){blocked=true;break}}if(!blocked)out.push(v)}
  level.vehicles=out;
  return level
}
